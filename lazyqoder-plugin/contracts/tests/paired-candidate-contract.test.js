...
const HOSTS = ['qoder-cli', 'qoder-ide', 'qoder', 'trae-cli', 'trae-ide', 'trae-work'];
...
function fixture() {
  ...
  writeFile(root, 'LazyQoder/lazyqoder-v1.2.2.tar.gz', 'buddy archive');
  ...
  const candidate = {
    ...
    products: [
      product('lazyqoder', 'LazyQoder', 'lazyqoder-v1.2.2.tar.gz'),
...
test('advertises a fresh-extraction LazyQoder verification command', () => {
  ...
  const inventory = [{
    path: 'LazyQoder/lazyqoder-v1.2.2.tar.gz',
    mode: '0644',
    size: 1,
    sha256: '2'.repeat(64),
  }];
  // When the paired candidate records the product self-verification contract.
  const product = productRecord('lazyqoder', 'LazyQoder', 'lazyqoder-v1.2.2.tar.gz', SHA, inventory);
