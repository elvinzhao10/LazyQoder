# LazyQoder Project Rules

> These rules are injected into agent context by Qoder at session start.
...
- Use the installed LazyQoder skills and commands as the workflow source of truth.
...
- Store active plans in `.lazyqoder/plans/`; keep loop state in `.lazyqoder/loop/`.
- Keep LazyQoder configuration, state, and evidence in `.lazyqoder/`.

### Preserve LazyQoder Semantics
...
- Document any deviation from LazyQoder semantics.
...
- Preserve evidence in `.lazyqoder/evidence/` and runtime state in `.lazyqoder/state/` when the workflow uses them.
...
- Qoder integration: `.qoder/`
- LazyQoder configuration and evidence: `.lazyqoder/`
- Plans and loop state: `.lazyqoder/plans/` and `.lazyqoder/loop/`
