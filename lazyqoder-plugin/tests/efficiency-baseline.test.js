...
test('rejects fixture when gate outcomes are omitted', (t) => {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-efficiency-'));
...
test('rejects fixture whose product does not match the runner', (t) => {
  ...
  // Then
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /product.*runner expects lazyqoder/);
});
...
test('rejects assertion symlink resolving into sibling product namespace', (t) => {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-efficiency-'));
  t.after(() => fs.rmSync(tempDir, { recursive: true }));
  const ownRoot = path.join(tempDir, 'lazyqoder-liveeval');
  ...
  fs.copyFileSync(path.join(evalRoot, 'lazyqoder-liveeval/runs/direct-20260828T0348/repo/tests/test_parse_duration.txt'), siblingFile);
  const linkedFile = path.join(ownRoot, 'linked-test.py');
  fs.symlinkSync(siblingFile, linkedFile);
  fixture.quality.assertion_manifest[0].file = 'lazyqoder-liveeval/linked-test.py';
