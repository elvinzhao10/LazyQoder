...
const PRODUCTS = {
  ...
  LazyQoder: 'https://github.com/elvinzhao10/LazyQoder.git',
};

function fixture(product = 'LazyQoder') {
...
test('cross-device promotion failure leaves the old active release readable', (t) => {
  ...
  expectCode(() => promoteRelease(f.paths, {
    ...
    origin: PRODUCTS.LazyQoder, runtimePath: process.execPath, version: '1.0.3',
  }), 'CROSS_DEVICE');
