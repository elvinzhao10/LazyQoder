...
function fixture() {
  const sandbox = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'lazyqoder lifecycle v2 '));
  ...
  return {
    sandbox,
    sourceRoot,
    paths: prepareProductRoot({ installRoot: path.join(sandbox, 'install'), product: 'LazyQoder' }),
  };
}
