...
grep -Fx -- '-sS --proto =https --proto-redir =https --max-redirs 0 --max-time 20 -A lazyqoder-docs/1.2.2 https://registry.npmjs.org/@scope/name/latest' "$CALLS" >/dev/null
...
assert_registry_only 1
grep -Fx -- '-sS --proto =https --proto-redir =https --max-redirs 0 --max-time 20 -A lazyqoder-docs/1.2.2 https://pypi.org/pypi/fastapi/json' "$CALLS" >/dev/null
