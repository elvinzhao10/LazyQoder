...
function writeFixtureFiles(root, selfTest = "process.stdout.write('self-test-ok\\n');\n") {
  ...
  fs.writeFileSync(path.join(packageRoot, '.qodercli-plugin', 'plugin.json'), '{"name":"lazyqoder","version":"1.3.3"}\n');
  fs.writeFileSync(path.join(packageRoot, '.qoder-plugin', 'plugin.json'), '{"name":"lazyqoder","version":"1.3.3"}\n');
...
function bootstrap(f, overrides = {}) {
  ...
  try {
    return bootstrapRelease(f.paths, {
      sourceUrl: 'https://github.com/elvinzhao10/LazyQoder/tree/main',
      ...overrides,
    });
  } finally {
...
test('parses only canonical official HTTPS source forms for the selected product', () => {
  ...
  const rejected = [
    ...
    'https://github.com/private/LazyQoder',
    'git@github.com:elvinzhao10/LazyQoder.git',
    '/tmp/LazyQoder',
    'https://github.com/elvinzhao10/LazyQoder/tree/../../main',
    'https://github.com/elvinzhao10/LazyQoder\n--upload-pack=owned',
  ];
  for (const [input, ref] of accepted) {
    assert.deepEqual(parseOfficialSource(input, 'LazyQoder'), {
      canonicalOrigin: OFFICIAL,
      product: 'LazyQoder',
      ref,
      repository: 'elvinzhao10/LazyQoder',
    });
  }
  for (const input of rejected) expectCode(() => parseOfficialSource(input, 'LazyQoder'), 'INVALID_ORIGIN');
});
...
test('repo, tag, branch, and full-SHA sources resolve through Git to the same immutable commit', () => {
  ...
  assert.equal(bootstrap(f, {
    sourceUrl: `https://github.com/elvinzhao10/LazyQoder/tree/${expectedSha}`,
  }).commit_sha, expectedSha);
});
...
test('same version at a different SHA requires an exact revision confirmation', () => {
  ...
  fs.appendFileSync(path.join(f.source, 'lazyqoder-plugin', 'scripts', 'lazyqoder-lifecycle.js'), "// v2\n");
...
test('manifest, checksum, self-test, prerequisite, and clone failures preserve active state', async (t) => {
  ...
  for (const [name, source] of [
    ...
    await t.test(name, () => {
      const f = fixture();
      fs.writeFileSync(path.join(f.source, 'lazyqoder-plugin/scripts/lifecycle-self-test.js'), source);
      git(f.source, ['add', '.']);
...
test('failed fresh bootstrap leaves a reusable scaffold for a later successful bootstrap', () => {
  ...
  assert.deepEqual(fs.readdirSync(f.paths.installRoot).filter((entry) => entry.startsWith('.LazyQoder-')), []);
});
...
test('failed bootstrap preserves a caller replacement installed before creator ownership capture', (t) => {
  ...
  t.mock.method(fs, 'mkdtempSync', (...args) => {
    ...
    if (!callerTarget && path.basename(created).startsWith('.LazyQoder-bootstrap-')) replace(created);
    return created;
  });
...
test('bootstrap refuses an exact-empty caller product root without changing it', (t) => {
  const sandbox = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'lazyqoder empty caller '));
  t.after(() => fs.rmSync(sandbox, { recursive: true, force: true }));
  const installRoot = path.join(sandbox, 'install root');
  const paths = productPaths({ installRoot, product: 'LazyQoder' });
  fs.mkdirSync(paths.productRoot, { recursive: true });
  const before = identity(paths.productRoot);

  expectCode(() => prepareBootstrapProductRoot({ installRoot, product: 'LazyQoder', timeoutMs: 50 }), 'WORKSPACE_PRESERVED');
...
test('cleanup never relocates either caller when a second caller occupies the public root', (t) => {
  const sandbox = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'lazyqoder two callers '));
  t.after(() => fs.rmSync(sandbox, { recursive: true, force: true }));
  const paths = productPaths({ installRoot: sandbox, product: 'LazyQoder' });
  ...
  t.mock.method(fs, 'renameSync', (source, target) => {
    if (!injected && source === paths.productRoot && path.basename(target).startsWith('.LazyQoder-cleanup-')) {
      injected = true;
  ...
  assert.deepEqual(fs.readdirSync(sandbox).filter((entry) => entry.startsWith('.LazyQoder-cleanup-')), []);
});

test('bootstrap preparation collision retry is bounded by its timeout', (t) => {
  const sandbox = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'lazyqoder collision timeout '));
  t.after(() => fs.rmSync(sandbox, { recursive: true, force: true }));
...
fs.mkdirSync = (target, ...args) => {
  if (target === path.join(root, 'LazyQoder')) { const error = new Error('collision'); error.code = 'EEXIST'; throw error; }
...
try {
  require(${JSON.stringify(modulePath)}).prepareBootstrapProductRoot({ installRoot: root, product: 'LazyQoder', timeoutMs: 40 });
  process.exitCode = 2;
} catch (error) {
...
test('dirty source bytes, local transport bypass, and mismatched confirmations fail closed', () => {
  const f = fixture();
  const entrypoint = path.join(f.source, 'lazyqoder-plugin/scripts/lazyqoder-lifecycle.js');
  fs.writeFileSync(entrypoint, "console.log('dirty-untrusted')\n");
  const clean = bootstrap(f);
  const installed = path.join(f.paths.releases, clean.release_id, 'lazyqoder-plugin/scripts/lazyqoder-lifecycle.js');
