...
function gitWorkspace(root) {
  ...
  git(workspace, ['config', 'user.email', 'lazyqoder@example.invalid']);
  git(workspace, ['config', 'user.name', 'LazyQoder Test']);
...
test('a requested worktree allocates an empty task-owned directory without claiming Git worktree isolation', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-worktree-allocation-'));
  try {
...
test('an untracked caller file blocks mutation allocation and is preserved', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-untracked-root-'));
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-untracked-workspace-'));
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(root, 'lazyqoder')), false);
  } finally {
...
test('release preserves a non-empty worktree allocation', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-worktree-preservation-'));
  const acquiredAt = 1_700_000_000_000;
  try {
...
test('expired-lease recovery preserves a non-empty worktree allocation', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-worktree-recovery-'));
  const acquiredAt = 1_700_000_000_000;
  try {
...
test('release preserves a real Git worktree that this allocator did not create or verify', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-real-worktree-'));
  let workspace;
  let worktree;
  try {
...
test('release preserves a symlink substituted for the task-owned allocation', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-symlink-'));
  try {
...
test('a malformed worktree ownership lease fails closed', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-malformed-owner-'));
  try {
...
test('release preserves a worktree recreated after its empty check', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-release-recreate-'));
  try {
...
test('expired recovery preserves a worktree recreated after its empty check', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lazyqoder-isolation-recovery-recreate-'));
  const acquiredAt = 1_700_000_000_000;
  try {
