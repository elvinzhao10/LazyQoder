...
if [ ! -f "$PLUGIN_ROOT/.qoder-plugin/plugin.json" ] \
    ...
    printf '%s\n' \
        'ERROR: LazyQoder plugin root is unavailable; keep this script under the v1.2.2 lazyqoder-plugin/scripts directory.' >&2
    exit 1
fi
...
try:
    ...
    if work_manifest.get("name") != "lazyqoder" or work_manifest.get("version") != version:
        raise ValueError("Qoder manifest must identify lazyqoder version 1.2.2")
    ...
    if entry is None or entry.get("version") != version or entry.get("source") != "./lazyqoder-plugin":
        raise ValueError("release marketplace must contain lazyqoder 1.2.2 from ./lazyqoder-plugin")
